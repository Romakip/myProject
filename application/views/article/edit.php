...
<form id="editArticle" method="post" action="<?= $Url::link("admin/article/edit&id=" . $_GET['id'])?>">
    <h5>Article Title</h5> 
    <input type="text" name="name" placeholder="наименование категории" value="<?= $viewArticle->title?>"><br>
    <h5 class="unput">Article Summary</h5>
    <textarea type="description" name="description" placeholred="описание"  value=><?= $viewArticle->summary ?></textarea>
    <h5>Article Content</h5>
    <textarea type="description" name="description" placeholred="контент"   value=><?= $viewArticle->content ?></textarea>
